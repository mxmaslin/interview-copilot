...
| Действие | Что делает |
|----------|------------|
| Старт `copilot` | Сессия + hotkeys `⌘↩`, `⌘G` (транскрипт сброшен) |
| **Начать / остановить прослушивание** | STT: интервьюер (BlackHole) + я (микрофон) |
| **Ответ на последний вопрос** (`⌘↩`) | Ответ в **терминале** + `data/last-answer.md` |
| **Микрофон на созвоне выкл** | ⌘↩ также на реплики `[Я]` |
| **Очистить транскрипт** (`⌘G`) | Сброс реплик в `data/transcript.md` |
| **Скриншот** (`⌘⌃⇧4` / пункт меню) | Vision-решение → терминал |
| **Открыть transcript / ответы** | Файлы в Finder / редакторе |
| **Выход** | Завершить sidecar (**Ctrl+C** в терминале) |
...
Секреты — в `.env` (в репозитории только `.env.example`). Резюме и `context/resume.hh-url` — **только локально** (шаблоны `*.example`).

Перед первым push, если в истории уже светился реальный resume ID:
...
Проверка: `cd sidecar && pytest tests/test_repo_hygiene.py -q` · [SECURITY.md](SECURITY.md)

## Без sidecar-ответа (только транскрипт)

1. Допиши реплику в `data/transcript.md` (`[Интервьюер]:` / `[Я]:`).
...
Покрыто: config, transcript, STT, Telegram, streaming answers, screenshot (очередь, параллельность с ⌘↩, provider fallback, clipboard), cursor model resolve, resume import, audio (~107 тестов). **Нет** e2e: реальные API, микрофон, `cursor agent` UI.
