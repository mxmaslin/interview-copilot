...
macOS-приложение в menubar для технических интервью: слушает созвон (STT), собирает транскрипт, по hotkey выдаёт короткий ответ в терминал. Опционально — vision по скриншоту задачи.
...
- **Резюме** — `context/resume.md` для фактов об опыте (локально, не в git); правила в `.cursor/rules/copilot.mdc` если открываешь репо в Cursor.
...
data/                    # transcript, last-answer (gitignored)
