...
- **Rolling STT** — live-текст в терминал (~1.2 с); **финал** в RAM после паузы (без записи диалога на диск). См. [audio-setup.md](docs/audio-setup.md).
- **Транскрипт** звонка — `[Интервьюер]:` (BlackHole) и `[Я]:` (микрофон).
- **Авто-ответ** — после финальной реплики в RAM (`ANSWER_AUTO=1`); повторный `⌘↩` перезапускает ответ (см. ограничения для `deepseek` ниже).
...
- **Архив сессий** — `data/sessions/<дата>/`: `review.md`, `turns.jsonl`, `meta.json` (диалог в RAM, не на диск).
...
При **`deepseek`**: не жмите **⌘↩** повторно, пока идёт стрим ответа в терминале — иначе два текста могут перемешаться (архив и `last-answer.md` при этом корректны). Подробнее: [docs/copilot-workflow.md](docs/copilot-workflow.md#ограничения-и-ux-по-полевым-прогонам).
