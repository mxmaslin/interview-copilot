...
- `ANSWER_AUTO=1` — ответ сразу после **финальной** реплики в transcript (`ANSWER_AUTO_DELAY_SEC=0`); обрезок `[Я]` после `STT_PENDING_FLUSH_SEC` тоже триггерит авто-ответ; **⌘↩** перезапускает ответ (предыдущий ход → `cancelled`). Мусор STT (эхо prompt, «Смешка») **не** запускает ответ.
...
## Отмена ответа и провайдер

| `ANSWER_PROVIDER` | Повторный ⌘↩ во время стрима |
|-------------------|------------------------------|
| `deepseek`, `openai` | Архив `cancelled`; `last-answer.md` защищён; **терминал** может смешать два стрима до конца HTTP |
| `cursor` | Subprocess SDK отменяется |

Не спамить ⌘↩ на тот же вопрос; на смену темы — дождаться конца ответа или **⌘G**. Подробно: [docs/copilot-workflow.md](docs/copilot-workflow.md#ограничения-и-ux-по-полевым-прогонам).
