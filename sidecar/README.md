...
macOS menubar **CP**: с запуска `copilot` — сессия интервью, `⌘↩` / `⌘G`, сброс диалога в RAM. **Rolling STT** (`STT_ROLLING=1`): live в терминал, финал в RAM (очередь STT: финал раньше rolling). Voice pipeline: endpointing, barge-in (`ANSWER_BARGE_IN_ON_SPEECH`), glossary, тайминги — [voice-pipeline.md](../docs/voice-pipeline.md). Hotkeys: **`⌘⌃⇧4`** (очередь скринов). **⌘↩** и скрины параллельно при `deepseek` + `cursor` для vision.
