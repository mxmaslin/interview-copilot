...
WHISPER_MODEL_SIZE=medium     # пресет fast/balanced; quality → large-v3
...
Первый запуск скачает модель **medium** (пресет `fast`, ~1.5 GB) — один раз. Дальше сегменты на машине, **без API-ключей**.
...
| `WHISPER_MODEL_SIZE` | `medium` (fast/balanced) | Явно переопределяет пресет; `quality` → `large-v3` |
| `WHISPER_BEAM_SIZE` | 1 | Меньше beam → быстрее |
| `WHISPER_VAD_FILTER` | 0 | VAD в Whisper дублирует сегментацию listener |
| `WHISPER_PROMPT_MODE` | `general` | `general` — HR/созвон; `interview` — IT-термины в prompt |
| `TERMINAL_SHOW_SELF` | `1` | Печатать реплики `[Я]` в терминал (микрофон) |
...
| Whisper `medium` int8 | 2–5 с / сегмент | CPU; `small` быстрее, хуже RU |
...
|----------------------|--------|
| `medium` | пресет `fast` / `balanced` (по умолчанию) |
| `large-v3` | `STT_LATENCY=quality`, максимум точности |
| `small` | если `medium` тяжело по CPU |
| `base` / `tiny` | только для экспериментов, хуже RU |

### Русская речь + английские термины (GIL, Redis, …)

Whisper склонен писать термины **русскими буквами** («гил», «редис»). В sidecar:

1. **Расширенный `initial_prompt`** — список EN-терминов латиницей (см. `stt_prompt.py`).
2. **`WHISPER_GLOSSARY_FIXES=1`** (по умолчанию) — пост-замена частых ошибок после STT.
3. Свой словарь: `WHISPER_INITIAL_PROMPT="…"` в `.env` — добавь термины своей вакансии.
4. Если мало: `WHISPER_MODEL_SIZE=medium` или `STT_LATENCY=quality` (медленнее, точнее).
5. Отключить глоссарий: `WHISPER_GLOSSARY_FIXES=0` — только сырой Whisper.
