...
| `AUDIO_SILENCE_SEC_SELF` | ~0.62 | ~0.68 | Микрофон `[Я]`; `call` ~0.78; явный env перебивает |
| `AUDIO_PRESET` | — | `interview` | Ещё короче пауза на микрофоне (~0.62 с) |
...
**Почему долго после фразы:** (1) тишина `AUDIO_SILENCE_SEC_SELF`, (2) очередь STT (финал впереди rolling), (3) Whisper beam на CPU. Ускорение: `STT_LATENCY=fast`, `AUDIO_PRESET=interview`, `STT_FAST_FINAL=1`.
