...
При `ANSWER_PROVIDER=deepseek` STT **не останавливается** на время ответа (`ANSWER_PAUSE_AUDIO=auto`). На время **скриншота** (Cursor SDK) STT тоже не стопается при deepseek; при `cursor` для ⌘↩ — стопается.
