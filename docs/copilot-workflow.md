...
| **Показать диалог в терминале** | Реплики из RAM в терминал |
| **Выход** | Остановить sidecar |

Ручной ввод реплик — через Telegram или STT; просмотр диалога — **CP → Показать диалог в терминале**. См. `AGENTS.md`.
