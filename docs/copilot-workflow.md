...
## Ограничения и UX (по полевым прогонам)

### Повторный ⌘↩ во время стрима (`deepseek` / `openai`)

Повторный **⌘↩** помечает предыдущий ход в архиве как `cancelled` и не даёт устаревшему тексту попасть в `data/last-answer.md` (`answer_turn` / `superseded`). Но **HTTP-стрим DeepSeek/OpenAI не обрывается** — старые чанки могут продолжать печататься в терминал **вперемешку** с новым ответом.

| Провайдер | Отмена in-flight в терминале |
|-----------|------------------------------|
| `deepseek`, `openai` | ⚠️ стрим может дойти до конца; дождитесь конца ответа или не жмите ⌘↩ снова |
| `cursor` | ✓ `cancel_active_sdk()` прерывает subprocess |

**Практика:** на новый вопрос — дождаться окончания стрима или **⌘G** и одна реплика; не спамить ⌘↩ на тот же вопрос (дубли ответа по Kafka и т.п.).

### Соло-репетиция (только Brio)

Если в транскрипте нет `[Интервьюер]` (нет BlackHole / Telegram), `ANSWER_SELF_QUESTIONS=auto` включает ответы на `[Я]`; в `timing` будет `speaker=self`. Перед реальным созвоном проверьте схему **BlackHole + Multi-Output** — см. [audio-setup.md](audio-setup.md).

### Типичные метрики (DeepSeek, hotkey)

`llm_ttft` ~0.7–1.0 s, `total` ~1.5–3 s — норма для озвучивания. Hint `stt в timing пуст` при ⌘↩ на **live** без финала — ожидаемо (ответ по rolling, финал в RAM после паузы).

### STT на речи

Опечатки вроде «прокавка» (proxy), «Такая погода» (Какая) — модель часто восстанавливает по контексту; для терминов см. [stt-glossary.md](stt-glossary.md).

Дорожная карта: abort HTTP-стрима при повторном ⌘↩ — [voice-pipeline-roadmap.md](voice-pipeline-roadmap.md).
